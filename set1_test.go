...
func TestDecrypt_AES_ECB_FromBase64File(t *testing.T) {
	...
	got, err := DecryptAESECBMode(EncryptedText{ciphertext: decoded, key: []byte(key)})
	if err != nil {
...
func TestDetectAESECBModeFromFile(t *testing.T) {
	...
	got, err := DetectAESECBMode(lines)
	if err != nil {
		t.Errorf("DetectAESECBMode(%q) threw an error: %s", filename, err)
	}
	if len(got) == 0 {
