...
func TestDecrypt_AES_ECB_FromBase64File(t *testing.T) {
	...
	got, err := Decrypt_AES_ECB_FromBase64File(in, key)
	if err != nil {
		t.Errorf("DecryptRepeatingKeyXorFromBase64(%q) threw an error: %s", in, err)
	}
	if got.plaintext != want {
		t.Errorf("DecryptRepeatingKeyXorFromBase64(%q) == %q, want %q", in, got.plaintext, want)
	}
}
...
func TestDetectAESinECBFromFile(t *testing.T) {
	...
	got, err := DetectAESinECBFromFile(in)
	if err != nil {
		t.Errorf("TestDetectAESinECBFromFile(%q) threw an error: %s", in, err)
	}
	if got[0].hexString != want {
		t.Errorf("TestDetectAESinECBFromFile(%q) == %q, want %q", in, got, want)
	}
}
