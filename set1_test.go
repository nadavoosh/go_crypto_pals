...
func TestHexFixedXor(t *testing.T) {
	in1 := HexEncoded{hexString: "1c0111001f010100061a024b53535009181c"}
	in2 := HexEncoded{hexString: "686974207468652062756c6c277320657965"}
...
func TestDecryptRepeatingKeyXor(t *testing.T) {
	...
	lines, err := ReadBase64File(filename)
	if err != nil {
		t.Errorf("ReadBase64File(%q) threw an error: %s", filename, err)
	}
	got, err := DecryptRepeatingKeyXor(lines)
	if err != nil {
		t.Errorf("DecryptRepeatingKeyXorFromBase64(%q) threw an error: %s", lines, err)
	}
	if string(got.key) != wantKey {
		t.Errorf("DecryptRepeatingKeyXorFromBase64(%q) == %q, want %q", lines, got.key, wantKey)
	}
	if string(got.plaintext) != funkyMusicPlain {
		t.Errorf("DecryptRepeatingKeyXorFromBase64(%q) == %q, want %q", lines, got.plaintext, funkyMusicPlain)
	}
}
...
func TestDecrypt_AES_ECB_FromBase64File(t *testing.T) {
	...
	got, err := DecryptAESECB(EncryptionResult{ciphertext: decoded, key: []byte(key)})
	if err != nil {
		t.Errorf("Decrypt_AES_ECB_b64(%q) threw an error: %s", decoded, err)
	}
	if string(got.plaintext) != FunkyMusic {
		t.Errorf("Decrypt_AES_ECB_b64(%q) == %q, want %q", decoded, got.plaintext, FunkyMusic)
	}
}

func TestDetectAESinECBFromFile(t *testing.T) {
	...
	got, err := DetectAESinECB(lines)
	if err != nil {
		t.Errorf("DetectAESinECB(%q) threw an error: %s", lines, err)
	}
	if got[0].hexString != want {
		t.Errorf("DetectAESinECB(%q) == %q, want %q", lines, got, want)
	}
}
